...
MODEL_NAME_1 = "pizza_steak_sushi_model_1.pth"
...
class_names_augmented = train_data_augmented.classes
class_names_simple = train_data_simple.classes


NUM_CLASSES = len(class_names_augmented)
accuracy_fn = Accuracy(task='multiclass', num_classes=NUM_CLASSES).to(device)

INPUT_SIZE_AUGMENTED = train_transform_trivial.transforms[0].size[0]
INPUT_SIZE_SIMPLE = simple_transform.transforms[0].size[0]

# Initiate model instances
model_0 = TinyVGG(input_shape=3,
                  hidden_units=16,
                  input_size=INPUT_SIZE_SIMPLE,
                  output_shape=len(class_names_simple)).to(device)

# Initiate model instances
model_1 = TinyVGG(input_shape=3,
                  hidden_units=16,
                  input_size=INPUT_SIZE_AUGMENTED,
                  output_shape=len(class_names_augmented)).to(device)

# Load the state dictionary

print(f"Loading model 0 state dict from: {MODEL_SAVE_PATH_0}")
model_0.load_state_dict(torch.load(f=MODEL_SAVE_PATH_0))

# Load state dict for model_1
print(f"Loading model 1 state dict from: {MODEL_SAVE_PATH_1}")
model_1.load_state_dict(torch.load(f=MODEL_SAVE_PATH_1))
